...
t_color		phong(t_item *data, t_color shadow)
{
	t_light		*light;
	t_color	somme;
	t_color	pixel;//phong_s;

	ft_bzero(&data->phong, sizeof(t_phong));
	light = data->light;
	while (data->light != NULL)
	{
		data->objet->light = vector_sub(data->light->pos, data->objet->point);
		data->phong.diffuse = color_add(data->phong.diffuse, ft_diffuse(data));
			data->light = data->light->next;
	}
	data->phong.diffuse = color_scal(255, data->phong.diffuse);
	data->phong.ambiant = color_scal(255, ft_ambient(data));
	somme = color_create(data->phong.diffuse);
	pixel = color_prd(shadow, somme);
	pixel = color_add(pixel, data->phong.ambiant);
	data->light = light;
	pixel = min_max(pixel);
	return (pixel);
}
